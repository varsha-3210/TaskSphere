...
export default function UserList({ team, setTeam }) {
  const { data, isLoading } = useGetTeamListQuery({ search: "" });
  ...
  // useEffect(() => {
  //   if (team?.length < 1) {
  //     setSelectedUsers(data?.filter((u) => team.includes(u._id)) || []);
  //   } else {
  //     setSelectedUsers([]);
  //   }
  // }, [isLoading]);
  ...
  useEffect(() => {
    if (!isLoading && data) {
      const preselected = data.filter((u) =>
        team?.some((t) => t._id === u._id || t === u._id)
      );
